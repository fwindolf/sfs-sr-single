...
params = {
    ...
    "gamma" : 1.0,
    "nu" : 0.0001,
    "mus" : [0, 0.0001, 0.001, 0.01, 0.1, 0.2, 0.5, 1, 2, 5, 10]
}


sql_gammas = """ 
    SELECT 
        mu, mean_error, rms_error, time, steps 
    FROM 
        runs 
    WHERE
        kappa = ? AND
...
fig.suptitle(r'Influence of $\mu$ parameter on quality and runtime')
fig.set_figheight(4)
fig.set_figwidth(15)
...
plt.savefig(os.path.join(output_dir, 'mu.png'))
